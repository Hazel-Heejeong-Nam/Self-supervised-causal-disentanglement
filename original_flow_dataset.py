...
if not os.path.exists(os.path.join(base,'flow_noise')): 
    os.makedirs(os.path.join(base,'flow_noise/train'))
    os.makedirs(os.path.join(base,'flow_noise/test'))
...
for r in range(5, 35):
    for h_raw in range(10, 40):
        for hole in range(6, 15):
            ...
            epsilon = 0.01 * np.max([np.abs(np.random.randn(1)),np.array([1])])
            ...
            plt.axis('off')
            if count == 4:
                plt.savefig(base+'/flow_noise/test/a_' + str(int(r))+"_"+str(int(h))+"_"+str(int(x_true*10))+"_"+str(int(hole))+'.png',dpi=96)
                count = 0
            else:
                plt.savefig(base+'/flow_noise/train/a_' + str(int(r)) + "_" + str(int(h)) + "_"+str(int(x_true*10))+"_"+str(int(hole))+'.png',dpi=96)
                count += 1
            plt.clf()
