...
def main_worker(args):
    ...
    args.model_name = f'{datetime.date.today().strftime("%m%d%Y")}_{args.sup}_data_{args.dataset}_z{args.z_dim}_c{args.concept}_obs_{args.observer}_Llr_{args.l_lr}_Clr_{args.c_lr}_labelbeta_{args.l_beta}_epoch_{args.epoch}_dagweights_{args.l_dag_w1}_{args.l_dag_w2}_{args.c_dag_w1}_{args.c_dag_w2}'
    ...
    model, static = train(args, train_loader, test_loader, disc, model, optimizer_D, optimizer_L, optimizer_C)          
    model.eval()
    ...
    for j in range(0,5):
        ...
        for i in range(args.concept):
            with torch.no_grad():
                c_rec_loss, c_kl_loss, c_recon_img, mask_loss = model(img,gt, static, mask=i, sample=sample, adj=j, beta=args.c_beta, info= args.sup,stage=1)
    ...
    txt_name = os.path.join(args.output_dir, f'summary_lr.txt')
    with open(txt_name, 'a' if os.path.isfile(txt_name) else 'w') as f:
        f.write(f'===================== LR-lable: {args.l_lr} || LR-causality: {args.c_lr} =======================\n')
        f.write(args.model_name)
        f.write(f'DAG SCORE: {dag_score:.6f}\n')
...
def parse_args():
    ...
    # observer 
    parser.add_argument('--l_lr', default=0.00003, type=float, help='learning rate for updating observer part')
    ...
    # interpreter  
    parser.add_argument('--c_lr', default=0.00003, type=float, help='learning rate for updating interpreter part')
