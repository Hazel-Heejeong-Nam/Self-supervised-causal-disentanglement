...
for i in range(-40,44):#pendulum
    for j in range(60,148):#light
        ...
        plt.axis('off')
        if count == 4:
          plt.savefig('/mnt/hazel/data/causal_data/pendulum/test/a_' + str(int(i)) + '_' + str(int(j)) + '_' + str(int(shade)) + '_' + str(int(mid)) +'.png',dpi=96)
          count = 0
        else:
          plt.savefig('/mnt/hazel/data/causal_data/pendulum/train/a_' + str(int(i)) + '_' + str(int(j)) + '_' + str(int(shade)) + '_' + str(int(mid)) +'.png',dpi=96)
        plt.clf()
        count += 1
