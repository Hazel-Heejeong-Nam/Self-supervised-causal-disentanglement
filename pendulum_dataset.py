...
if not os.path.exists(os.path.join(base,'pendulum')): 
    os.makedirs(os.path.join(base,'pendulum/train'))
    os.makedirs(os.path.join(base,'pendulum/test'))
...
for i in range(-40,44):#pendulum
    for j in range(60,148):#light
        ...
        plt.axis('off')
        if count == 4:
          plt.savefig(base+'/pendulum/test/a_' + str(int(i)) + '_' + str(int(j)) + '_' + str(int(shade)) + '_' + str(int(mid)) +'.png',dpi=96)
          count = 0
        else:
          plt.savefig(base+'/pendulum/train/a_'+ str(int(i)) + '_' + str(int(j)) + '_' + str(int(shade)) + '_' + str(int(mid)) +'.png',dpi=96)
        plt.clf()
        count += 1
